...
export function registerConsoleLogs(server: McpServer) {
  server.tool(
    'get_console_logs',
    'Get browser console messages (errors, warnings, logs) captured since the browser was opened or last drained. Reveals silent JS failures that break UX.',
    {
      drain: z
        .boolean()
        .optional()
