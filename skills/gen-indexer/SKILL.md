...
This skill builds `CLAUDE.md` **from scratch**. If one already exists and just needs refreshing to match a changed codebase, use [`update-indexer`](../update-indexer/SKILL.md) instead — it re-verifies sections in place rather than regenerating the whole file.

## Triggered by

- User typing `/gen-indexer`
...
> Generated by devexp `/gen-indexer` on <YYYY-MM-DD>. Update this file when conventions change — or run `/update-indexer` to refresh it in place.
