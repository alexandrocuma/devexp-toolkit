...
- **A Kimi install says what it did *not* install (#112, #113).** Only hooks are left, so every run that selected Kimi ends with a line naming them (`Kimi Code CLI: hooks are not installed for it yet (#114).`), repeated in the summary of a multi-target run. It also says that `./uninstall.sh` cannot remove a Kimi install yet (#115), because telling you what was written without telling you it cannot be removed cleanly is half the story. #111's rule that a run which installed nothing exits non-zero no longer applies to Kimi at all: every `--*-only` flag now installs what it names, so no combination leaves a Kimi run empty-handed, and a Kimi-only run succeeds where on `0.9.4` it always failed.
...
- **Kimi Code CLI is a selectable target (#111).** It can be picked in the wizard or with `--target kimi`. When #111 landed it wrote nothing at all; this release fills that in — MCP servers from #112, agents and skills from #113 — leaving hooks (#114). So that a partial install cannot be mistaken for a complete one, every Kimi run names what it did not install, and a multi-target run repeats it in the summary.
