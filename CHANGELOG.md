...
- **A Kimi install now writes everything devexp ships (#112, #113, #114).** MCP servers, agents, skills and hooks all install, so the "what it did not install" notice — and the machinery behind it — is gone. What remains is the missing way out: every run still says that `./uninstall.sh` cannot remove a Kimi install yet (#115), because telling you what was written without telling you it cannot be removed cleanly is half the story. #111's rule that a run which installed nothing exits non-zero no longer applies to Kimi at all: every `--*-only` flag installs what it names, so no combination leaves a Kimi run empty-handed, and a Kimi-only run succeeds where on `0.9.4` it always failed.
