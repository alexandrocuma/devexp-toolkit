...
- **A second install no longer registered every Kimi guard twice (#114).** Kimi rewrites `config.toml` whenever it writes its own config — on login above all — and its writer emits no comments, so devexp's `# devexp:hooks:begin` / `# devexp:hooks:end` markers disappear while the `[[hooks]]` entries between them stay. The next install, finding no markers, appended its block: every guard registered twice, every tool call scanned twice, every block reason printed twice. When no markers are present, devexp now recognises its own entries by their adapter command and rewrites them as one freshly marked block instead. Only when there are none — a file that still has its markers is the authoritative answer to which lines are devexp's, and an entry you wrote outside the block to run the adapter your own way stays yours. Removal reaches the unmarked entries too, so a machine you have logged in on does not keep a live guard after the block is taken out.
