...
- **A `$KIMI_CODE_HOME` that cannot be written into a prompt is refused (#113).** From this release the resolved Kimi root is written into the installed agent and skill bodies, which are prompts a model reads — so a root holding a newline or an escape sequence could put attacker-chosen lines inside every installed agent's instructions, and one holding bytes that are not valid UTF-8 could not survive the round trip, leaving the rewritten reference pointing at nothing (the same class as #140). Both are now refused before anything is read or written, at the single gate every Kimi destination is built from. macOS already rejects such a directory name; Linux does not.
