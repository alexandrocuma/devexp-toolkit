...
- **Kimi Code CLI installs MCP servers (#112).** Selecting Kimi — in the wizard or with `--target kimi` — now merges the registry's MCP servers into the `mcpServers` object of `$KIMI_CODE_HOME/mcp.json` (default `~/.kimi-code/mcp.json`). Kimi has no `mcp add` command, so devexp edits the file directly, and it is a file you also edit:
  ...
  - **Your entries stay yours.** devexp records a fingerprint of every entry it writes in `$KIMI/.devexp-manifest.json`, and rewrites or removes only entries that still match it. An entry you wrote — including one that happens to be identical to devexp's, which is never adopted — or one of devexp's that you have since edited, is left exactly as it is and said to be; `--reinstall-mcps` replaces it. An entry devexp wrote is removed only once it is no longer installed — deselected, gone from the registry, or made project-scoped by it — the way a stale agent file is. Unrelated top-level keys, your key order and your numbers all survive unchanged.
  - **A second install writes nothing** and reports `already configured`, leaving the file byte-identical.
  - **`${VAR}` is resolved at install time**, because Kimi expands nothing when it reads the file — not `${VAR}`, not `$VAR`, not `~`. An MCP whose required env vars (or placeholders) are unset is skipped with the usual `[REQUIRED]` notice and its setup instructions, never written half-configured — and an entry devexp wrote on an earlier run stays exactly where it is, because a variable you have not exported in this shell is not a request to uninstall anything.
  - **The file cannot be left in a state Kimi refuses.** Every entry is checked against Kimi's schema before it goes in, and the whole file is re-parsed and re-checked before it replaces anything. An `mcp.json` devexp cannot parse — including one with comments, which `mcp.json` does not allow even though opencode's `config.json` does — is left untouched with a warning naming the servers to add by hand, and the rest of the install carries on. This matters more here than for the other targets: Kimi rejects the **whole file** if a single entry in it is invalid, leaving you with no MCP servers at all and only a line in `$KIMI/logs/kimi-code.log` to say why — so an invalid entry devexp finds already in your file is reported too.
  - A new `mcp.json` is created `0600` (entries hold resolved values from `mcps/.env`); an existing one keeps its mode, and a symlinked one keeps the link and updates the file it points at.
  - `--dry-run` writes nothing and names each change; `--mcps-only` works, and saves the manifest, because for Kimi the manifest is the ownership record.

  `kimi doctor` validates `config.toml` and `tui.toml` only and never looks at `mcp.json` — to check the result, start `kimi` and run `/mcp`.

- **A Kimi install says what it did *not* install (#112).** Agents and skills arrive in #113, hooks in #114, so every run that selected Kimi ends with a line naming them, repeated in the summary of a multi-target run. #111's rule that a run which installed nothing exits non-zero is kept where it is still true: asking a Kimi-only run for `--agents-only` or `--skills-only` installs nothing and exits non-zero rather than reporting `All done.` A Kimi-only run that installs its MCP servers now succeeds, where on `0.9.4` it always failed.
...
- **Kimi Code CLI is a selectable target (#111).** It can be picked in the wizard or with `--target kimi`. When #111 landed it wrote nothing at all; #112 (above) gives it MCP servers, and agents, skills and hooks follow in #113-#114. So that a partial install cannot be mistaken for a complete one, every run names what it did not install, a multi-target run repeats it in the summary, and a run whose every target installed nothing exits non-zero instead of printing `All done.`
- **`devexp install --target kimi` installs agents and skills too (#113).** Alongside the MCP servers of #112, the run backs up anything already there, writes the transformed agents and the skill directories, records all three kinds in one `$KIMI/.devexp-manifest.json`, and removes what an earlier run installed and this release no longer ships — under the same rules as the other targets: never through a symlinked entry (#124), never through a symlinked target directory or from behind one (#128), and whatever could not be removed stays in the manifest so a later run retries. `--dry-run` writes nothing, each of `--mcps-only` / `--agents-only` / `--skills-only` installs exactly its own kind, and the summary names only the destinations that run actually wrote to. `--model` is ignored with a warning, because Kimi has no per-agent model.
  **Hooks (#114) are still missing**, so every Kimi run ends by saying so; `./uninstall.sh` cannot remove a Kimi install yet either (#115). A Kimi-only run now succeeds — it installs three of the four kinds — where before #113 asking it for agents or skills alone installed nothing and exited non-zero.
...
- **Any combination of install targets, and a `--target` flag (#111).** `devexp install --target claude,kimi` installs for exactly those CLIs; the flag is repeatable and comma-separated. When more than one CLI is detected the interactive prompt is now a checklist rather than a three-way "Claude Code / opencode / Both" choice, so any combination of the three can be picked. Deselecting everything is refused instead of being read as "install for all", and asking for a CLI that is not installed is refused instead of quietly falling back to another.
- **Kimi Code CLI became a selectable target (#111).** It can be picked in the wizard or with `--target kimi`. At the time this landed it installed nothing; agents and skills arrive later in this same release (#113), and MCP servers (#112) and hooks (#114) are still to come. So that it cannot be mistaken for a successful install, the run names the directory it left untouched, a multi-target run repeats it in the summary, and a run whose every target installs nothing exits non-zero instead of printing `All done.`
...
- **A Kimi install that fails partway keeps its record of what it already wrote (#113).** The manifest is the only thing that tells devexp's `mcp.json` entries from the user's. Now that agents and skills install after the MCP step, a failure in either would have returned with `mcp.json` already merged and its ownership unrecorded — and the next run would have read devexp's own entries as the user's and never updated or removed them again. The record is written whichever step fails.
