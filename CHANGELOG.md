...
- **Kimi Code CLI installs MCP servers (#112).** Selecting Kimi — in the wizard or with `--target kimi` — now merges the registry's MCP servers into the `mcpServers` object of `$KIMI_CODE_HOME/mcp.json` (default `~/.kimi-code/mcp.json`). Kimi has no `mcp add` command, so devexp edits the file directly, and it is a file you also edit:
  ...
  - **Your entries stay yours.** devexp records a fingerprint of every entry it writes in `$KIMI/.devexp-manifest.json`, and rewrites or removes only entries that still match it. An entry you wrote, or one of devexp's that you have since edited, is left exactly as it is and said to be; `--reinstall-mcps` replaces it. An entry devexp wrote and no longer installs is removed, the way a stale agent file is. Unrelated top-level keys, your key order and your numbers all survive unchanged.
  - **A second install writes nothing** and reports `already configured`, leaving the file byte-identical.
  - **`${VAR}` is resolved at install time**, because Kimi expands nothing when it reads the file — not `${VAR}`, not `$VAR`, not `~`. An MCP whose required env vars (or placeholders) are unset is skipped with the usual `[REQUIRED]` notice and its setup instructions, never written half-configured.
