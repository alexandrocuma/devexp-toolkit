...
- **The documented install one-liner did not work.** `curl -fsSL .../remote-install.sh | bash` failed for anyone who did not set `DEVEXP_VERSION`, which is everyone following the README. The release lookup piped curl into `grep -m1`; `-m1` closes the pipe at the first match while curl is still writing the response, so curl died of SIGPIPE — `curl: (56) Failure writing output to destination` — and `set -o pipefail` turned that into an abort two lines into the run.

  The lookup now reads the response into a variable and parses it from a here-string, which bash backs with a temporary file, so no writer is left to kill. The trap is worth naming precisely, because the first attempt at this fix reproduced it exactly one step later: moving the early-quitting matcher from `grep` to `sed` just meant `printf` took the SIGPIPE instead. It is not `grep -m1` that is unsafe — it is *any* matcher that stops early on the reading end of a pipe.

  **Why nothing caught it.** `remote-install.test.sh` pins `DEVEXP_VERSION` in every case, so no test entered the lookup branch, and its stub `curl` exits 22 immediately — it could not have produced a body long enough to trigger the SIGPIPE anyway. The release guide's post-release verification pins the version too. Two independent checks, both avoiding the one path users actually take. The suite now covers it, with a stub that answers the API call with ~2MB so a matcher that stops early is guaranteed to close the pipe mid-write; with a short body curl finishes first and the bug hides.

- **Two more docs still described a three-job CI.** `docs/guides/workflows.md` listed the required checks as `test`, `hooks`, `govulncheck` and told the reader to "add `lint` when it lands" — it landed in #207 — and `docs/development/testing.md` opened by saying CI runs in three jobs. Both are corrected. The guard that would have caught all of this is #217; these two are fixed now rather than left as its fixture, because the repo proves guards by mutation (`scripts/smoke.sh`) and does not need live drift to demonstrate one.

- **The release guide still described a three-job CI.** Five places named `test`, `hooks` and `govulncheck` and left out `lint`, which #207 added: the called-workflow description, the Build block's inline job list, the failure-triage bullet that says which job failing means the tag won't ship, the automated gate in Target: cli → Promote, and the post-release verification row listing what the run holds. The same pass also corrected "the three script suites" — the `hooks` job runs four (claude-code, kimi, opencode, installer-script).

  Found by reading the guide against `ci.yml` after cutting v0.11.0, not by a check. A job list written in prose has no guard, which is the same reason the branch-protection claims went stale in the first place.
...
- **Two unchecked type assertions could panic mid-uninstall** (#194). `kimiReadDirNames` asserted `os.Root.FS()` into a `ReadDir` interface, and the opencode legacy-plugin path asserted `before["plugin"]` to `[]any` after a *textual* splice that never proved the value was a JSON array. Both were bare assertions, so a config holding a string or an object there would have panicked the uninstall and left the user's config half-edited with no message they could act on. Both now use comma-ok and degrade: the first reports an error, the second leaves the file untouched — which is that function's own stated rule, since a shape it cannot read semantically is precisely a case where it cannot prove the splice equals the semantic edit.

  Both were found by `check-type-assertions`, which is not `errcheck`'s default. A type assertion that fails yields the zero value; that is the same failure-as-success in a different costume.
...
- **The fail-closed guards are measured against their own scan budget** (#197) — `hooks/claude-code/scan-latency.test.sh` and `hooks/opencode/scan-latency.test.js`, six inputs each, both twins. `scan-budget.test.sh` already proved the *mechanism* — a forced budget hit blocks, the budget cannot be switched off from the environment, an over-ceiling value is clamped — but every one of those cases uses a *forced* budget. Nothing asserted that a real guard, on realistic input, finishes inside the real one. The 15000 ms default was "sized from the worst cases measured" once and never re-checked, while `secret-guard`, `secret-in-write-guard` and `dangerous-cmd-guard` have all grown since and two open tickets propose widening their pattern sets further.

  This is a correctness gate, not a performance one. A budget hit **exits 2 — a block**. So a guard that grows past its budget does not get slower for the user; it starts **denying legitimate tool calls**, citing a budget they did not set. The suites therefore fail at **half** the budget, not at the budget: crossing it should be a red build naming the guard while 2x of real budget is still in hand, rather than a user's blocked call. Every run prints the percentage consumed, pass or fail, so growth shows as a trend before it is a failure.

  Non-flakiness is the actual work, since a gate that cries wolf on a busy runner gets deleted. Each measurement is best-of-three — a scheduler can only make a run slower, so the fastest observation is the least contaminated — the limit is a fraction of the budget rather than a figure in milliseconds, and the budget is read out of `scan-budget.sh` rather than copied, so a test cannot keep passing after the budget is lowered underneath it. The JS suite additionally fails if the two twins' defaults disagree at all: a budget enforced on one side only means one CLI blocks where the other allows, on identical input.

  Measured headroom, recorded where the default's comment claims the measurement was taken: 7% of budget for a 400 KB pipeline through `dangerous-cmd-guard`, 5% for a 1 MB command, 4% for a 1 MB command through `secret-guard`, 2% for a 2 MB write through `secret-in-write-guard`, 1% for an ordinary `Read`. The two suites are not comparable to each other and say so — a Claude Code hook is a fresh process per tool call and really does pay bash and python3 startup every time, while the opencode plugin loads once and runs in-process thereafter.
...
- **The comment standard is enforced in every language, not just Go.** The `lint` job gains a second step, and `docs/development/setup.md`, `testing.md`, `conventions.md` and `CLAUDE.md` stop claiming lint is unenforced — a claim that survived the job being added.

  **`.golangci.yml` gains `revive`'s `exported` rule and `gofmt`**, joining the `errcheck` and `nolintlint` that landed with the no-silent-success work. A new undocumented exported symbol now fails, and formatting never reaches a review comment. Deliberately narrow still — the default linter set is off, and widening is a decision rather than a default.

  Turning `gofmt` on found one file where two Go toolchains disagree. A map literal in `cli/internal/hooks/installer_test.go` has one key far longer than its neighbours, sitting on `go/printer`'s alignment-group threshold — and which side it falls on changed between Go versions, so `gofmt`'s verdict depended on the toolchain golangci-lint was built with. Formatting it either way made the other complain; there was no fixed point. The group is now split deliberately with a blank line and a comment saying why, which both versions format identically.

  Dropping the `cli/internal/assets` path exclusion was the interesting part: that directory is *not* generated. `assets.go` and `assets_test.go` are tracked source, and only the trees they embed are staged, so excluding the path had been hiding a genuinely bare exported symbol. `assets.FS` is now documented — including that the `all:` prefix on each embed directory is load-bearing, since without it embed skips entries beginning with `.` or `_` and would silently drop dotfiles the install needs.
...
- **The exported API of `cli/` is documented** — 38 declarations across 11 files, plus a package doc comment on each of the seven packages that had none (`cmd`, `agents`, `config`, `hooks`, `mcp`, `skills`, `ui`). `revive`'s `exported` rule now passes across `cli/` with `cli/internal/assets/` excluded, which is what #189 needs in order to turn that rule on without turning CI red.
  ...
  The ten one-line `ui` wrappers were the case the ticket called out as easy to pad. They get a package doc carrying the system — two levels of line, heading versus indented item, and which stream each goes to — and one short line each stating only what distinguishes that one, rather than ten restatements of its own name.

  The ticket's own inventory was short by 15: it listed functions, and `revive` also flags exported types, exported methods on exported types, and exported const blocks. `Config`, `MCP`, `Registry`, `Hook`, `Action`, `Scope`, `Execute`, `LoadDotenv`, `InstallOpencode`, `ConfigRefusedError.Error` and the two `iota` blocks in `ui/prompts.go` were all found by running the rule rather than reading the list.
...
- **Repo consistency is a test, not a checklist** (#193). `cli/internal/repocheck/` asserts the coupling that `docs/guides/workflows.md` could only ask people to remember — it names the step itself: "the `opencode` and `kimi` mappings in step 3 are the ones people miss... Without the `opencode` mapping, opencode silently lacks the hook." Eight assertions now fail `go test` instead: a registry entry naming a file that does not exist (in either direction, including the `kimi` script, since Kimi reads a hook it cannot start as an *allow*); a hook on disk that no registry entry names; a `kimi` block switched off without a reason; a hook no test mentions; an agent or skill missing from its catalog, or catalogued twice, or catalogued without existing; and every count in prose that no longer matches disk.

  The counts were **all correct** when this was written, in all nine places. That was the problem — nothing made them correct, so `CLAUDE.md`'s own gotcha list carried "Adding or removing an agent, skill or hook leaves counts stale". A `countClaim` table pins each sentence and the failure names the file and what to edit; a second sweep fails on any *unpinned* count in the index and catalog documents, so a new sentence stating a number is caught the first time rather than the next time it drifts.

  Three exclusions are deliberate and commented where they are made. `CHANGELOG.md` is never swept: "14 agents" in an old entry was true when written, and correcting history to match today is the opposite of what a changelog is for. Asset bodies are never swept: `agents/synthesis.md` says "at least 2 agent reports", which instructs the model and counts nothing. And `agents/opencode/orchestrator.md` stays out of the "34" structurally — it is catalogued as `opencode/orchestrator.md`, with a slash, so neither the row pattern nor the directory walk sees it, which is what keeps the exception from being something anyone has to remember.

- **`hooks/opencode/large-file-guard.js` has tests** (#193) — 35 cases. It had none; the bash twin had 133 lines. The gap was found by the coverage assertion above on its first run, which is the point: the guard has shipped untested on opencode since it was written, and nothing said so. The new suite pins the decision boundary (501 lines blocks, exactly 500 allows), the tools the guard must leave alone (`edit` is exempt by design — a guard that nagged on every edit to a long file gets switched off, and a disabled guard guards nothing), and that a new or missing path is not an overwrite. The shell twin's injection cases do not transfer — there is no shell here — but hostile file names are still exercised, because the thrown message embeds the name and a mangled one is a name the user cannot recognise.
...
- **`./uninstall.sh` removes a Kimi Code CLI install (#115).** The last thing a Kimi install could not do was undo itself. It can now: agents, skills, the `mcp.json` entries devexp owns, the `config.toml` hooks block, the copied guard scripts and the manifest all come out in one step. Unlike Claude Code and opencode, whose agents the script matches by repo filename, **nothing in the Kimi root identifies devexp's files except the manifest**, so the whole removal happens in the hidden `devexp uninstall --target kimi`, which the script finds the way it already finds the opencode one (`DEVEXP_BIN`, then `bin/devexp`, then `PATH`) and previews with its `--dry-run`. Every step goes through the function the *install* uses, so the two cannot drift: the hooks come out through `UninstallKimi`, which takes the registration out **before** the scripts it points at — a registered command whose script is missing is a silent allow under Kimi's runner — and an `mcp.json` entry is removed only while its fingerprint still matches what devexp recorded, so **an entry you wrote, or one of devexp's you have since edited, stays** and stops being tracked. The same removal rules as everywhere else apply (#124, #128) — to the agents, the skills **and the copied hook scripts**, which had to be brought onto that path for it to be true: nothing through a symlinked directory or from behind one, never an entry that is itself a symlink, exact recorded names only, all through an `os.Root` handle — and the guard resolves from the Kimi root's *parent*, so it still works with a `$KIMI_CODE_HOME` outside your home directory. Whatever could not be removed stays in the manifest so a later run can finish, and the manifest itself goes only once nothing of devexp's is left. **Kept on purpose, and said so:** `$KIMI/.devexp-backup-*` (your pre-install copies) and `~/.claude/agent-memory`, which Claude Code shares.

- **A Kimi install says what Kimi honours less of (#115).** Everything devexp ships installs for Kimi, but four things behave differently enough that assuming Claude Code's behaviour leaves you wrong about what is protecting you, so every run that installs them now says so: **an agent's body replaces Kimi's whole system prompt** (which is why every installed agent appends `${base_prompt}`); **`allowed-tools` is ignored, so there is no per-skill tool restriction at all**; **skills appear only as `/skill:<name>` in Kimi's listings**, which is the form to type (a bare `/<name>` does resolve — Kimi falls back to the prefixed key — but nothing shows it to you); and **hooks fail open** — a hook that times out, cannot spawn or exits with anything but 0 or 2 is read as an allow. The last is a security property rather than an inconvenience, which is why it is in the terminal and not only in the docs. The full list — including `kimi doctor` validating `config.toml` and `tui.toml` only, so it can check neither skills nor `mcp.json`; a project `.mcp.json` shadowing a devexp entry; `~` expanding *before* the outside-workspace check; and unknown tool names being dropped with nothing in the log — is in [install.md](docs/guides/install.md#kimi-code-cli). The warning names only what that run installed, so `--mcps-only` does not warn about agents.

- **`./uninstall.sh` prompts for the CLIs it actually found (#115).** The fixed `[1] Claude Code / [2] opencode / [3] Both` menu became a numbered list of the installs detected, answered with comma-separated numbers or `a` for all — with three targets a fixed menu would need seven entries and would still offer CLIs with nothing installed. Every number is validated before any of them is acted on, so a typo in the second cannot remove the first.

  **If a script of yours pipes an answer into the uninstaller** (`printf '3\n' | ./uninstall.sh`), the numbers have changed meaning: `[3]` used to be *Both*, and on a machine where Kimi is also detected it now selects *Kimi Code CLI* alone. The list only ever holds the CLIs actually detected, so no fixed number is stable across machines — pass `--yes` to take all of them, or answer `a`.

- **`devexp install --target kimi` installs the guards (#114).** The run copies `hooks/kimi/adapter.sh`, every guard Kimi can honour and the `scan-budget.sh` those guards source into `$KIMI_CODE_HOME/hooks/`, keeping the executable bit, and only then registers the commands in `config.toml`. **Copied, not run from the checkout:** Kimi holds a plain shell command and reads a command it cannot run as an *allow*, so a moved or deleted clone would silently disarm every guard, and a registration that pointed at a script not yet on disk would do the same. Today that is `secret-guard`, `dangerous-cmd-guard` and `secret-in-write-guard`; each hook Kimi cannot honour — the on-save hooks, the graphify hooks, `large-file-guard` — prints its own reason on the run that would otherwise have installed it. `hooks.disabled`, the wizard's selection and `--dry-run` behave as they do for the other targets, every `--*-only` flag skips hooks as it does elsewhere, and a hook you switch off has its copied script removed along with its registration. What was installed is recorded in the manifest's `hooks` list, and a step that fails partway records what it already wrote and prunes nothing.

- **devexp's Kimi hooks live in one marked block of `config.toml` (#114).** The guards are registered as `[[hooks]]` entries between a `# devexp:hooks:begin` and a `# devexp:hooks:end` line in `$KIMI_CODE_HOME/config.toml`, and **only those lines are ever rewritten** — your providers, models, keys, your own `[[hooks]]`, your comments, your key order and your spacing all come back byte for byte, and a re-install that changes nothing does not touch the file at all. A guard that stops being installed leaves the block; removing the block restores the file you started with. The block is delimited by comments rather than by a key inside each entry because **Kimi's hook schema is strict**: one unknown key, one unknown event or one timeout outside `1..600` makes Kimi drop the *whole* hooks section — your own hooks with it — behind a diagnostic nobody reads. So every entry devexp writes is checked against that schema first, the rendered file is re-parsed and re-verified before it replaces anything, a hook devexp cannot write validly is skipped with a reason instead, and a hook of **yours** that Kimi already rejects is reported and left as it is. A `config.toml` devexp cannot parse, whose markers it cannot make sense of, or that it cannot replace safely (a dangling symlink, a read-only file) is **left untouched and reported**, never rewritten.

- **The guards run under Kimi Code CLI through an adapter (#114).** `hooks/kimi/adapter.sh` is what a Kimi `[[hooks]]` entry runs, with the Claude Code guard to run as its one argument. It translates Kimi's PreToolUse envelope into the one the guards read and carries the verdict back. Kimi's envelope is **snake_case at the top level and only there**: `runMatchedHooks` puts the payload through `toHookInputData` — a one-level `camelToSnake` — before `runHook` spawns the command, so `tool_name` and `tool_input` arrive already spelled the way the guards want, while `tool_input` keeps the tool's own argument names. The adapter accepts both spellings, snake_case first, renames a tool's `path` to `file_path` for Read, Write and Edit, maps `ReadMediaFile` onto `Read`, and passes every other tool name and every other argument through unchanged. **It is fail-closed in Kimi's direction:** Kimi reads exit 2 as a block and *everything else* — any other exit code, a spawn failure, a timeout — as an allow, so bad JSON, a missing or unusable `tool_name`/`tool_input` in either spelling, a missing or unreadable guard, a missing `python3`, and a guard exit that is neither allow (0) nor block (2) all end at exit 2 rather than letting an unscanned tool call through. An **`ask` verdict becomes a block** carrying the guard's own reason, because Kimi's hook runner honours only `permissionDecision: "deny"` and would otherwise turn a confirmation prompt into consent. The adapter never manufactures an allow: it mints neither the guards' scan proof nor the scan budget's, and it runs the guard with no arguments, so the budget (v0.9.3) and the proof-of-scan rule (v0.9.4) survive unchanged — a budget hit still arrives as a block.

- **Kimi Code CLI installs MCP servers (#112).** Selecting Kimi — in the wizard or with `--target kimi` — now merges the registry's MCP servers into the `mcpServers` object of `$KIMI_CODE_HOME/mcp.json` (default `~/.kimi-code/mcp.json`). Kimi has no `mcp add` command, so devexp edits the file directly, and it is a file you also edit:

- **A Kimi install now writes everything devexp ships (#112, #113, #114).** MCP servers, agents, skills and hooks all install, so the "what it did not install" notice — and the machinery behind it — is gone. What every run still says is what Kimi honours *less* of than Claude Code (#115), because telling you what was written without telling you how much of it takes effect is half the story. #111's rule that a run which installed nothing exits non-zero no longer applies to Kimi at all: every `--*-only` flag installs what it names, so no combination leaves a Kimi run empty-handed, and a Kimi-only run succeeds where on `0.9.4` it always failed.

- **`devexp install --target kimi` installs agents and skills too (#113).** Alongside the MCP servers of #112, the run backs up anything already there, writes the transformed agents and the skill directories, records all three kinds in one `$KIMI/.devexp-manifest.json`, and removes what an earlier run installed and this release no longer ships — under the same rules as the other targets: never through a symlinked entry (#124), never through a symlinked target directory or from behind one (#128), and whatever could not be removed stays in the manifest so a later run retries. `--dry-run` writes nothing, each of `--mcps-only` / `--agents-only` / `--skills-only` installs exactly its own kind, and the summary names only the destinations that run actually wrote to. `--model` is ignored with a warning, because Kimi has no per-agent model.

- **devexp skills are installed for Kimi Code CLI (#113).** All 8 skills land in `$KIMI_CODE_HOME/skills/<name>/` with their supporting files — `graphify/references/`, which opencode never receives — and Kimi does not mistake those subdirectories for skills of their own. Each `SKILL.md` is validated the way Kimi validates it (strict YAML, `name` and `description` required, a `type` it supports) and refused with a reason rather than installed if it would fail, because a skill Kimi drops from its catalog leaves nothing a user can diagnose. Body references to `~/.claude/agents/<name>.md` are repointed at the Kimi install. The rewritten `SKILL.md` is substituted while the directory is copied rather than written over it afterwards, so a symlinked `SKILL.md` inside a real skill directory is still left untouched (#124).

- **devexp agents are transformed for Kimi Code CLI (#113).** `$KIMI_CODE_HOME/agents/` gets all 34 agents with their tool names mapped onto Kimi's — `WebFetch` becomes `FetchURL`, the four `Task*` tools and `TodoWrite` collapse onto `TodoList`, and a name with no equivalent is dropped and **named in the output**, because Kimi drops an unknown tool silently and writes nothing to its log. `color`, `memory` and `model` are dropped, since Kimi ignores them. Each of the nine agents that can delegate gets an explicit `subagents` allowlist — every agent this run installed that cannot itself delegate, plus Kimi's `explore` and `plan` — so a delegation chain stays one level deep and a disabled agent is never offered. Body references to `~/.claude/agents/<name>.md` are repointed at the Kimi install — as `~/.kimi-code/agents/<name>.md` when `$KIMI_CODE_HOME` is unset, so **nothing derived from your environment is written into a file a model reads**, and as the absolute path when it is set, since a custom root has no tilde form. `~/.claude/agent-memory` is deliberately left alone, so both CLIs go on sharing one atlas. Front matter is rebuilt through a YAML parser rather than by splitting lines, so a description keeps its exact value whatever quoting it used.
...
- **Kimi Code CLI is a selectable target (#111).** It can be picked in the wizard or with `--target kimi`. When #111 landed it wrote nothing at all; this release fills that in completely — MCP servers from #112, agents and skills from #113, hooks from #114.
...
- **Kimi Code CLI target paths (#111).** `kimiTargetPaths` resolves the agents, skills and hooks directories, `mcp.json`, `config.toml`, the devexp manifest and the backup directory under `$KIMI_CODE_HOME`, defaulting to `~/.kimi-code`. The paths are resolved up front so a `$KIMI_CODE_HOME` devexp must not write to — relative, the filesystem root, the home directory itself, or anything containing the home directory — is refused now rather than discovered later. An unrelated absolute path such as `/opt/kimi` is deliberately allowed: removals stay guarded there because the guard root is the Kimi root's parent, not `$HOME`.

### Fixed

- **`./uninstall.sh --yes` could block on stdin for ever (#115).** `--yes` was read only as `$1`, so it was missed in any other position; and the target menu ran regardless of it, so a non-interactive uninstall on a machine with more than one install stopped at a prompt nothing would ever answer. It is now recognised anywhere in the arguments and skips the menu, removing from every detected CLI.

- **Every guarded tool call in Kimi was refused (#114).** devexp read Kimi's hook runner but not its caller, and the caller is where the payload is built: `runMatchedHooks` puts the whole top-level envelope through `toHookInputData` — a one-level `camelToSnake` — before `runHook` spawns the command. The adapter looked for `toolName`, a real envelope carries `tool_name`, and the adapter is fail-closed, so **every Read, ReadMediaFile, Bash, Write and Edit in a Kimi session was blocked with an internal error**. Nothing was ever let through unscanned, but Kimi was unusable with devexp's hooks installed and the only way out was to switch the guards off. The adapter now accepts both spellings, snake_case first. This never reached a release.

- **A re-install could not report a `config.toml` Kimi had stopped reading (#114).** A bare key written after devexp's end marker binds to devexp's last `[[hooks]]` table, and Kimi's strict hook schema then drops the *entire* hooks section — your own hooks and every devexp guard — behind a diagnostic nobody reads. In the session it just looks as though no hook exists, and the first thing anyone does is re-run the installer. The installer said nothing, because the marked region was unchanged and it returned before it looked. It now runs both checks on that path and reports without writing anything. This never reached a release.

- **A second install no longer registered every Kimi guard twice (#114).** Kimi rewrites `config.toml` whenever it writes its own config — on login above all — and its writer emits no comments, so devexp's `# devexp:hooks:begin` / `# devexp:hooks:end` markers disappear while the `[[hooks]]` entries between them stay. The next install, finding no markers, appended its block: every guard registered twice, every tool call scanned twice, every block reason printed twice. When no markers are present, devexp now recognises its own entries and rewrites them as one freshly marked block instead. Only when there are none: a file that still has its markers is the authoritative answer to which lines are devexp's. What counts as "its own" is the **whole command devexp renders, out of devexp's own hooks directory** — `bash '<hooks>/kimi/adapter.sh' '<hooks>/claude-code/<guard>.sh'` and nothing else — so a hook of yours with a different root, a guard from elsewhere, an extra argument or a wrapper of your own is left where it is, matcher and timeout intact. After one login there are no markers, so this is the ordinary path rather than a rare one, and what it matches it **deletes**: devexp says so in as many words and lists every command it took over, because a line you wrote by hand that happens to be byte for byte what devexp renders cannot be told from a previous install's. Removal reaches the unmarked entries too, so a machine you have logged in on does not keep a live guard after the block is taken out.

- **A Kimi install that fails partway keeps its record of what it already wrote (#113).** The manifest is the only thing that tells devexp's `mcp.json` entries from the user's, and the only record of which agent and skill files are devexp's. A failure used to discard it twice over: between steps, so a failing agent install lost the MCP ownership record written moments earlier; and inside a step, so a run that wrote 14 agents before one destination turned out to be a directory recorded none of them. Either way the next run would read devexp's own files as the user's — never pruning one you had since deselected, and leaving `./uninstall.sh` (#115) nothing to go on either, since that manifest is what it reads. The record is now written whichever step fails, and includes what a half-finished step managed to write. Nothing is pruned on that path: the install set is incomplete, so everything the step never reached would look stale.
- **A Kimi root that cannot be written into a prompt is refused (#113).** From this release the resolved Kimi root is written into the installed agent and skill bodies, which are prompts a model reads. Three shapes are refused before anything is read or written: a **control character** (a newline puts attacker-chosen lines inside every installed agent's instructions, at column 1), a **`${…}`, `$ARGUMENTS` or `$<n>` form** (Kimi substitutes exactly these when it renders an agent or skill body, so the repointed reference would become something else at the moment it is used), and **bytes that are not valid UTF-8** (they cannot survive the round trip, leaving the reference pointing at nothing — the same class as #140). The check covers the root whether it came from `$KIMI_CODE_HOME` or from `$HOME` via the default `~/.kimi-code`. macOS already rejects such a directory name; Linux does not.
...
- **A failed install no longer dumps the usage block or says everything twice (#111).** `devexp install` errors are printed once, by the same code that sets the exit status, and the flag list is shown only when a flag was actually wrong. It mattered most while selecting only a target that installed nothing yet was a deliberate non-zero exit — no target does that any more — and the notice explaining it was ending up three screens above the usage dump.
