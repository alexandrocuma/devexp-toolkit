...
- **`devexp install --target kimi` installs agents and skills, and says what it still cannot install (#113).** The run backs up anything already there, writes the transformed agents and the skill directories, records them in `$KIMI/.devexp-manifest.json`, and removes what an earlier run installed and this release no longer ships — under the same rules as the other targets: never through a symlinked entry (#124), never through a symlinked target directory or from behind one (#128), and whatever could not be removed stays in the manifest so a later run retries. `--dry-run`, `--agents-only` and `--skills-only` all work, and `--model` is ignored with a warning — the run continues; Kimi has no per-agent model.
  **It still exits non-zero for a Kimi-only run.** MCP servers (#112) and hooks (#114) are not installed yet and `./uninstall.sh` cannot remove a Kimi install yet (#115), so the run names what it wrote, names what is missing, and declines to print `All done.` The message that used to say *nothing was installed* now says the run is not a complete install, because after this change the first claim would be false.
