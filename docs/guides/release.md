...
- **`ci / govulncheck` exited 1 (infrastructure, not a finding)** — a fetch or network error (vuln.go.dev, the module proxy), marked `infrastructure failure, not a finding` in the log. Re-run: `gh run rerun <run-id> --failed`. This re-runs the failed `ci` jobs and then `goreleaser`, which still finds the draft.
- **`ci / test` or `ci / hooks` failed, `ci / govulncheck` exited 3 (called vulnerability), or `goreleaser` failed and can't be fixed by re-running** — this tag won't ship:
...
|--------|-------|--------------|
| Release workflow | `gh run list --workflow release.yml --limit 1` | `completed success` for the new tag — the run holds `ci / test`, `ci / hooks`, `ci / govulncheck` and `goreleaser` (`.github/workflows/release.yml`) |
