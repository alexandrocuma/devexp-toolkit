...
6. Run `./install.sh`, then check that the hook appears in `~/.claude/settings.json` (Claude Code) or in `~/.config/opencode/plugins/devexp/hooks.json` (opencode), and exercise both the block and allow paths.
...
- **Go module:** add or bump it in `cli/go.mod` with the Go module tooling (`go get`, `go mod tidy` in `cli/`), and commit `cli/go.mod` and `cli/go.sum` together. There's no written approval rule. In practice, bumps land in their own `fix:` commit whose body says why (drift or CVE) and confirms the upgrade had no impact (`e6f65e3` cobra/viper, `e147e67` Go toolchain `1.25.11` for stdlib CVEs). **Go toolchain:** the `toolchain` line in `cli/go.mod` is the one place the build's Go version is set. Local builds switch to it under `GOTOOLCHAIN=auto`, and both workflows install it through `go-version-file: cli/go.mod` (`.github/workflows/ci.yml`, `.github/workflows/release.yml`), so don't add a `go-version` there, and don't set `GOTOOLCHAIN` in workflow or job `env` (setup-go then ignores the `toolchain` line and falls back to the `go` directive). To pick up a stdlib fix, bump only the `toolchain` line to a supported, fixed patch and check with `govulncheck ./...` from `cli/`. Leave the `go` directive (the minimum language version) alone unless code needs a newer one.
