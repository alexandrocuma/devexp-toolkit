...
- **Hooks**:
  - Claude Code: new hooks in `hooks/registry.json` are added to `settings.json`; hooks already registered are left as-is, including ones disabled since. The one exception is the command string. Another spelling of one of this repo/cache dir's own scripts is rewritten in place to the form devexp writes, which is quoted only when the path needs it. That covers the bare path, the path in double quotes and a single-quoted path that needs no quoting. If the event already holds the registered command, the other spelling is removed instead, so the script doesn't run twice (see [Hook commands](#hook-commands-claude-code)). Nothing else in `settings.json` changes (see [What install and uninstall change in settings.json](#what-install-and-uninstall-change-in-settingsjson)).
