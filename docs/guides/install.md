...
- **Hooks**:
  - Claude Code: new hooks in `hooks/registry.json` are added to `settings.json`; hooks already registered are left as-is, including ones disabled since. There are two exceptions, the matcher and the command string. An enabled hook registered under another matcher than the registry's (the registry changed it since, say from `Write|Edit` to `Write|Edit|MultiEdit|NotebookEdit`) is brought to the registry's matcher (`registerHook`): an entry holding only that hook's command takes the new matcher in place and keeps its other fields; from an entry it shares with other commands, the handler moves out, fields and all, into an entry of its own, so the other commands keep their matcher. A hook already registered under the registry's matcher is left alone, even if its command also appears in another entry, and a disabled hook keeps whatever matcher it has. Another spelling of one of this repo/cache dir's own scripts is rewritten in place to the form devexp writes, which is quoted only when the path needs it. That covers the bare path, the path in double quotes and a single-quoted path that needs no quoting. If the event already holds the registered command, the other spelling is removed instead, so the script doesn't run twice (see [Hook commands](#hook-commands-claude-code)). Nothing else in `settings.json` changes (see [What install and uninstall change in settings.json](#what-install-and-uninstall-change-in-settingsjson)).
...
- **Only the value of the top-level `hooks` key is rewritten.** Every other byte of the file, other keys, their formatting and order, and the trailing newline, is kept. The `hooks` value is written in the indentation of the line its key is on (two spaces, four, tabs), or compact in a compact file. Install keeps string escapes and number spellings inside `hooks` as they were; `uninstall.sh` writes them as python's `json` does (`\u00e9` becomes `é`, `2.50` becomes `2.5`). A file without `hooks` gains the key after its last member; a new file is written as earlier releases wrote it. Events devexp adds go after the existing ones, sorted. Nothing is written when there's nothing to change.
- **A `settings.json` devexp can't edit without losing something is left untouched.** That's invalid JSON, a top level that isn't an object, a `hooks` that isn't an object, an event that isn't an array, or an entry or handler that isn't an object. `devexp install` stops with an error naming the file, and no hooks are registered, where earlier releases replaced the whole file with just the hooks. `uninstall.sh` skips the step with a message. Both also check that the rewritten file decodes to the original with only `hooks` changed, and write nothing otherwise.
