...
- **Kimi drops comments when it rewrites `config.toml`** — which it does on login — so devexp's markers disappear. When there are no markers at all, devexp recognises its own entries by their adapter command and re-marks them, rather than appending a second copy of every hook.
