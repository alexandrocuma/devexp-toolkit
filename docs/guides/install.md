...
The installer is CLI-agnostic. It detects which AI coding CLI(s) are installed and, when more than one is present, asks which to target (`cli/cmd/targets.go`). Supported CLIs: **Claude Code**, **opencode** and **Kimi Code CLI**. Kimi Code installs MCP servers, agents, skills and hooks — see [Kimi Code CLI](#kimi-code-cli) below.
...
Passing any of `--dry-run`, `--reinstall-mcps`, `--mcps-only`, `--agents-only`, `--skills-only` or `--target` skips the interactive wizard. With none of them the wizard runs — but only where there is a terminal to answer it (`chooseInstallMode` in `cli/cmd/install.go`). With no terminal and no flags, a bare `devexp install` says so and installs everything for every detected CLI rather than dying at the first prompt, which is what makes `curl … | bash` work (`scripts/remote-install.sh` runs a bare `devexp install` with stdin still on the pipe).

`--model` overrides the `model` value from `devexp.config.json` (`cli/cmd/install.go:118-120`). It does **not** skip the wizard — the wizard has no model prompt (`cli/cmd/wizard.go`) — so combine it with one of the flags above for a non-interactive run. It accepts a short alias (`sonnet`, `opus`, `haiku`, `gpt4o`, `deepseek`, `kimi`, …), resolved to a provider-prefixed ID such as `anthropic/claude-sonnet-4-6`, or any other string used verbatim (`modelMap` / `resolveModel` in `cli/internal/agents/installer.go`). The value only **replaces an existing `model:` frontmatter line**; agents without one — most of them (only `dep-audit`, `docs-sync` and `runbook` declare `model:` today) — get no model line, for both CLIs.
...
- The wizard's agent, MCP and hook checklists all start fully ticked. Unticking everything in one of them installs **none** of that kind — it is not read as "install them all" — while the other checklists are unaffected; the `Done` row shows the live count (`0 / 34 selected`) before you confirm (`MultiSelect` in `cli/internal/ui/prompts.go`, the resolvers in `cli/cmd/registry.go`). The target checklist is the exception: an empty pick there is an error, because installing for no CLI installs nothing at all
...
- **Kimi Code CLI**: merges MCPs into the `mcpServers` key of `$KIMI_CODE_HOME/mcp.json`, resolving `${VAR}` first because Kimi expands nothing; entries devexp did not write are never touched, an unchanged file is not rewritten, and an `mcp.json` devexp cannot parse is left alone with a warning. Agents, skills and hooks are installed too (`cli/cmd/install_kimi.go`, `cli/internal/mcp/kimi.go`, `cli/internal/hooks/kimi_install.go`)
...
- **Agents and skills**: devexp tracks what it installed in `~/.claude/.devexp-manifest.json` (and `~/.config/opencode/.devexp-manifest.json` for opencode). On each run, anything from the previous manifest that isn't part of this run's install set is removed from disk and dropped from the manifest. An entry left on disk behind a symlinked directory (below), or because it couldn't be checked or removed, stays recorded once, however often the manifest lists it, so a later run can finish the job; every other entry kept below is dropped.
  ...
  - An entry is removed only under the exact name devexp recorded. The directory is read once, and an entry that isn't listed there byte for byte is dropped from the manifest. If it is on disk under a spelling that differs only in case or Unicode normalization, which a case-insensitive filesystem such as default APFS resolves to the same file, it is kept with a warning: that file is someone else's, for example your own `Retired-Agent.md` after devexp's `retired-agent.md` was removed. A file you create under the exact recorded name can't be told apart from devexp's; move it aside before re-running if the name was devexp's.
  ...
  - **`agents/`, `skills/` or `commands/` is a symlink, or behind one** (for example managed from dotfiles, linked to a source checkout, or under a linked `~/.claude`, `~/.config` or `~/.config/opencode`) (#128):
    - devexp installs through it, as for `plugins/` below.
    - It never removes anything through it. The output says `"<dir>" is a symlink — devexp never removes files through it; remove these by hand: …`, or `"<dir>" is behind a symlink (it resolves to "<real dir>") — …`, listing each stale entry it would otherwise have removed. `--dry-run` prints the same line.
    - Those entries stay recorded in the manifest, so a run after the link is replaced with a real directory removes them.
    - "Behind a symlink" means the directory, resolved, is not `<resolved HOME>/<its path under HOME>`. HOME is resolved on both sides, so a symlink above it (macOS `/var` → `/private/var`, a linked `/home`) doesn't count. Every target is built from `HOME`; devexp reads no other config-directory variable.
    - Removals go through a handle on the directory that was checked (`os.Root`), so a directory swapped for a symlink while devexp runs can't redirect them. A swap between the check and opening the directory is detected, and nothing is removed from it that run.
  - A stale entry that names something this run installed is never removed: the same name apart from case (for example `DEV-AGENT.md` after a rename to `dev-agent.md`, which a case-insensitive filesystem such as default APFS treats as the same file), or the same file on disk (a hard link). It is kept with a warning. A variant that differs only in Unicode normalization isn't on disk under its exact name, so it isn't removed either.
...
- **opencode plugin files**: the opencode manifest's `plugins` key lists every plugin file installed (`devexp.js` first, then `devexp/…`).
  ...
  - **`plugins/` is a symlink to a directory, or behind one** (for example managed from dotfiles, or under a linked `~/.config` or `~/.config/opencode`; "behind" as for agents above):
    ...
    - The output says `plugins/ is a symlink — devexp never removes files through it; remove these by hand: …` (or `is behind a symlink (it resolves to …)`). Those files stay recorded in the manifest, so a run after the link is replaced with a directory cleans them up.
...
- Never removes an agent or skill through an `agents/` or `skills/` directory that is a symlink or behind one (resolved from `HOME`, as in [Stale-file cleanup](#stale-file-cleanup)), and never removes one that is itself a symlink. Before the confirmation it lists them instead: `<dir> is a symlink — devexp never removes files through it; remove these by hand:` (or `<dir> is behind a symlink (it resolves to <real dir>) — …`) followed by the paths, and `<path> is a symlink — left untouched (devexp never removes one)`. A symlinked entry is reported as a symlink even inside a symlinked directory (#128)
  ...
  - `Removed N item(s)` counts only what was actually removed; a failed `rm` is reported as `could not remove <path>`
- Skills (`~/.claude/skills/`) are only removed if uninstalling from all CLIs that use them
- **Claude Code hooks** are removed from `settings.json` by an embedded python step, with the install's rules: a registry script under a `hooks/claude-code/` path segment, as a plain path or one single-quoted absolute path, from any install root; a script in that form that is gone from `<root>/hooks/claude-code/` of any devexp install root (this repo or another, recognised by its `hooks/registry.json`, #150); plus this repo's own scripts spelled as the bare path or, when the path has no `$`, backquote, `\` or `"`, the path in double quotes ([Hook commands](#hook-commands-claude-code)). The file is saved atomically and through a symlink ([How devexp saves files](#how-devexp-saves-files)); a file it can't save, or one nested more than 500 levels deep, is skipped with a message and the uninstall carries on. Everything else is left in place, with every field and every byte outside `hooks` ([What install and uninstall change in settings.json](#what-install-and-uninstall-change-in-settingsjson)).
...
- **opencode MCP servers** are removed from `config.json` by an embedded python step, which never fails the uninstall:
  ...
  - one nested more than 500 levels deep is skipped with a message (#150);
...
`$KIMI` below is `$KIMI_CODE_HOME`, or `~/.kimi-code` when that is unset (`kimiTargetPaths` in `cli/cmd/paths.go`). MCP servers, agents, skills and hooks are all installed there.
...
| Hooks | `~/.claude/settings.json` (shell scripts) | `~/.config/opencode/plugins/devexp.js` + `devexp/` (selected modules, `utils.js`, `package.json`, `hooks.json`) | marked `[[hooks]]` block in `$KIMI/config.toml`, running `$KIMI/hooks/kimi/adapter.sh` + the copied guards |
...
An installed agent holds two shapes of path, and both are deliberate. References to other **agents** are repointed at the Kimi install: `~/.kimi-code/agents/<name>.md` when `$KIMI_CODE_HOME` is unset, and the absolute path when it is set, since a custom root has no tilde form. The tilde form is preferred where it exists because it puts nothing derived from your environment into a file a model reads. References to **`~/.claude/agent-memory`** are left exactly as they are, so a Kimi session and a Claude Code session share one atlas and one set of per-project notes rather than keeping two that drift apart. Seeing both forms in the same file is not a mistake.

> **Measured about Kimi 2.0.1, and useful to know when writing your own agents:** `~/` is expanded in the path argument of `Read`, `Write`, `Edit`, `Glob` and `Grep`, against `HOME` as Kimi sees it. Expansion happens *before* Kimi's absolute-path test, so `~/…` also clears the guard that refuses a **relative** path to a file outside the workspace — `~/.kimi-code/agents/x.md` is readable from a project directory where `../.kimi-code/agents/x.md` is refused with "not an absolute path". Separately, paths that look like secrets (`.env`, `id_rsa`, `credentials`, …) are blocked outright whatever form they take.
...
- **`${VAR}` is resolved at install time.** Kimi expands nothing when it reads *this file* — not `${VAR}`, not `$VAR`, not `~` — so a placeholder left in it would be spawned literally. (That is about `mcp.json` only. Paths a *tool* is given are a different code path, and `~/` is expanded there — see the callout above.)
- **Your own entries and keys are left exactly as they are.** devexp records a fingerprint of each entry it writes in the manifest, and rewrites or removes only entries that still match it. An entry you wrote — including one that happens to be identical to devexp's, which is never adopted — or one of devexp's you have since edited, is skipped and said to be; `--reinstall-mcps` is how you hand it back.
- **An entry devexp wrote is removed only when it is no longer installed** — when you deselect that MCP, when it leaves the registry, or when the registry makes it project-scoped, which devexp never writes to Kimi's user file — the way a stale agent file is removed. An MCP that is still selected but cannot be configured on this run, because a `required_env` variable is unset in this shell or this clone, keeps its entry and keeps its notice: "I cannot configure this now" is not "this is no longer installed".
...
- **Every entry is checked against Kimi's schema before the file is replaced** — including for a JSON `null` in a field, which Kimi refuses and Go would otherwise read as an empty value — and a file devexp cannot parse is refused untouched with a warning, the rest of the install carrying on. Both matter more here than elsewhere: Kimi rejects the *whole file* if a single entry is invalid, leaving you with no MCP servers at all and only a line in `$KIMI/logs/kimi-code.log` to say why. `mcp.json` is strict JSON — unlike opencode's `config.json` it is not JSONC, so a comment in it is a file Kimi cannot read either.
...
**Hooks** are the guards devexp ships for Claude Code, run through an adapter and registered as a marked `[[hooks]]` block in `$KIMI/config.toml` (`cli/internal/hooks/kimi_install.go`, `kimi_config.go`, `hooks/kimi/adapter.sh`):

- **The scripts are copied into `$KIMI/hooks/`**, not run out of the checkout. Kimi holds a plain shell command, and a command it cannot run is read as an **allow** — so a moved or deleted clone would silently disarm every guard. `kimi/adapter.sh`, the selected guards and `claude-code/scan-budget.sh` (which each guard sources from its own directory) all land there, and are copied *before* anything is registered.
- **Only the guards Kimi can honour are installed.** A `PostToolUse` hook's result is never read, and `permissionDecision: "ask"` is run as an allow, so the on-save hooks, the graphify hooks and `large-file-guard` are off for Kimi — each with its reason printed on the run that would otherwise have installed it. What is installed today: `secret-guard`, `dangerous-cmd-guard` and `secret-in-write-guard`.
- **Your own `[[hooks]]` are left alone**, as are your providers, models, keys, comments and key order: only the lines between devexp's two marker comments are rewritten. A `config.toml` devexp cannot parse, or whose markers are unbalanced, is refused untouched.
- **Kimi drops comments when it rewrites `config.toml`** — which it does on login — so devexp's markers disappear. When there are no markers at all, devexp recognises its own entries and re-marks them, rather than appending a second copy of every hook. It recognises them by the **whole command it renders, out of its own hooks directory** (`bash '<hooks>/kimi/adapter.sh' '<hooks>/claude-code/<guard>.sh'`, and nothing else), so an entry of yours with a different root, a different guard, an extra argument or a wrapper of your own is left exactly as it is. What it does match, it **takes over and deletes**, and it names every command it took over — a line you wrote by hand that is byte for byte what devexp renders cannot be told from a previous install's.
- **A run that has nothing to write still checks the file.** A hook of yours that Kimi rejects, or a stray bare key that has bound itself to devexp's last entry, makes Kimi drop every hook in the file; re-running the installer is what most people do when the guards go quiet, so that run reports what it found even though it changes nothing.
- **An entry Kimi's schema would reject is never written.** One invalid entry makes Kimi drop the *whole* hooks section, yours included, behind a warning nobody reads; a hook already in the file that Kimi rejects is left as it is and warned about.
- `hooks.disabled` in `devexp.config.json`, and the wizard's hook selection, both apply. A hook you switch off has its script removed as well as its registration.
