...
- **An entry devexp wrote is removed only when it is no longer installed**, i.e. when you deselect that MCP or it leaves the registry — the way a stale agent file is removed. An MCP that is still selected but cannot be configured on this run, because a `required_env` variable is unset in this shell or this clone, keeps its entry and keeps its notice: "I cannot configure this now" is not "this is no longer installed".
