...
- **Agents and skills**: devexp tracks what it installed in `~/.claude/.devexp-manifest.json` (and `~/.config/opencode/.devexp-manifest.json` for opencode). On each run, anything from the previous manifest that isn't part of this run's install set is removed from disk and dropped from the manifest. An entry left on disk behind a symlinked directory (below), or because it couldn't be checked or removed, stays recorded so a later run can finish the job; every other entry kept below is dropped.
  ...
  - **`agents/`, `skills/` or `commands/` is a symlink** (for example managed from dotfiles, or linked to a source checkout) (#128):
    - devexp installs through it, as for `plugins/` below.
    - It never removes anything through it. The output says `"<dir>" is a symlink — devexp never removes files through it; remove these by hand: …`, listing each stale entry it would otherwise have removed. `--dry-run` prints the same line.
    - Those entries stay recorded in the manifest, so a run after the link is replaced with a real directory removes them.
    - Only the directory itself is checked. When `~/.claude` or `~/.config` is the symlink, the `agents/` inside it is a real directory and is cleaned up as usual.
