...
**Behavior:**
- Refuses to run when `HOME` is unset, empty or not an absolute path — before it resolves assets, opens the wizard, registers MCPs or writes/backs up anything — because every destination below is built from it and would otherwise land under the current directory (`targetHome` in `cli/cmd/paths.go`, checked first in `runInstall`)
