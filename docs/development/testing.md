...
At this commit: 10 Go packages with tests; `dangerous-cmd-guard.test.sh` 160 cases, `fail-closed.test.sh` 10, `secret-guard.test.sh` 37; `dangerous-cmd-guard.test.js` 161, `devexp-plugin.test.js` 130, `secret-guard.test.js` 38; `uninstall.test.sh` 94, `install.test.sh` 17, `remote-install.test.sh` 24.
...
- **Hook tests** (`secret-guard.test.sh`, `dangerous-cmd-guard.test.sh`, and their `.js` mirrors):
  ...
  - JS: header `mirrors hooks/claude-code/<hook>.test.sh`, import the module's exported pure predicate (`isSecretFile`/`secretInCommand` from `secret-guard.js`, `blockReason` from `dangerous-cmd-guard.js`, which applies `BLOCK_PATTERNS` to the `maskInert` text the hook scans), loop over block/allow arrays, print `N passed, M failed`, `process.exit(fail === 0 ? 0 : 1)`.
