...
- **Reference test to copy:**
  ...
  - `hooks/kimi/runner.test.sh` — the shape for a property of another CLI's *runner* rather than of a hook: it reproduces how Kimi Code CLI 2.0.1's `runHook` spawns a registered command (`sh -c <command>` with no arguments, the camelCase envelope on stdin, exit 2 as the block) and drives the real adapter and the real guards through it. Copy this shape when running the real CLI would touch the developer's own credentials or state; say in the header what is and is not simulated, and cite the bundle code it mirrors.
