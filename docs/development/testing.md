...
- **Hooks without behaviour tests:** `secret-in-write-guard` and `large-file-guard` are covered only by `fail-closed.test.sh` (shell side); `format-on-save`, `lint-on-save` and `test-on-save` by that plus `on-save-path.test.sh` and its mirror `hooks/opencode/on-save-path.test.js`, which pin which file each tool is called on and its exact argv with stub tools on `PATH`, not what the tool does; the three `graphify-*` hooks have no tests at all; on the opencode side `secret-guard` and `dangerous-cmd-guard` have their own `.test.js` files, and `devexp-plugin.test.js` covers the entry plus the lint/format/test-on-save edit handlers only as far as they run with no linter, formatter or test runner found (no tool output is asserted).
