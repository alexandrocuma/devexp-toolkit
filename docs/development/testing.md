...
bash hooks/kimi/adapter.test.sh                                    # the Kimi adapter
...
- **New test file path:**
  ...
  - Hooks: `hooks/claude-code/<hook>.test.sh` **and** a mirrored `hooks/opencode/<hook>.test.js` covering the same cases — e.g. `secret-guard.test.sh` / `secret-guard.test.js`, `dangerous-cmd-guard.test.sh` / `.test.js`. CI picks new files up by glob (the four `for f in …` loops in `ci.yml`'s `hooks` job); nothing to register.
- **Reference test to copy:**
  ...
  - `hooks/kimi/runner.test.sh` — the shape for a property of another CLI's *runner* rather than of a hook: it reproduces how Kimi Code CLI 2.0.1 runs a registered command — `matchHooks.ts`'s `toHookInputData` over the top-level payload, then `runHook`'s `sh -c <command>` with no arguments, the result on stdin, exit 2 as the block — and drives the real adapter and the real guards through it, once per envelope spelling. Copy this shape when running the real CLI would touch the developer's own credentials or state. **Reproduce the caller, not just the callee:** the first version of this file mirrored `runHook` alone, fed the pre-conversion camelCase payload, and passed 7/7 against a shape Kimi never sends while the adapter refused every real tool call. Say in the header what is and is not simulated, cite the bundle code it mirrors, and assert the built envelope's shape so the simulation cannot drift.
