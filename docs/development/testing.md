...
At this commit: 10 Go packages with tests; `dangerous-cmd-guard.test.sh` 242 cases, `fail-closed.test.sh` 10, `interpreter-isolation.test.sh` 11, `large-file-guard.test.sh` 34, `on-save-path.test.sh` 73, `secret-guard.test.sh` 37, `secret-in-write-guard.test.sh` 159; `dangerous-cmd-guard.test.js` 279 (incl. timing checks on crafted long lines), `devexp-plugin.test.js` 130, `on-save-path.test.js` 61, `secret-guard.test.js` 38, `secret-in-write-guard.test.js` 141; `uninstall.test.sh` 128, `install.test.sh` 21, `remote-install.test.sh` 24.
