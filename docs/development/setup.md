...
No lint, formatter, scanner or release tool needs to be installed locally: `scripts/golangci-lint.sh` and `scripts/govulncheck.sh` each install their pinned tool into a temporary directory that is removed on exit, `scripts/check-comment-refs.sh` needs only python3, and goreleaser runs only in GitHub Actions (see [`../guides/release.md`](../guides/release.md)).
...
| Test — single hook | `bash hooks/claude-code/secret-guard.test.sh` · `bash hooks/kimi/adapter.test.sh` · `node hooks/opencode/secret-guard.test.js` | `Run:` header line in each test file |
...
| Lint — Go | `./scripts/golangci-lint.sh` — gofmt + revive's `exported` rule over `cli/`, pinned golangci-lint installed to a temp dir. Exit 1 = finding, 2 = infrastructure | `.golangci.yml`, `scripts/golangci-lint.sh`, `.github/workflows/ci.yml` (job `lint`) |
...
| `KIMI_CODE_HOME` | No | `~/.kimi-code` | Root of the Kimi Code install destinations. Must be absolute, and may be neither `/` nor `$HOME`; anything else is refused. `mcp.json`, the agents, skills and hooks directories, `config.toml`'s devexp hook block and the manifest are all written there. It is also refused when it holds a control character, a `${…}`/`$ARGUMENTS`/`$<n>` form or invalid UTF-8, because it is written into the installed agent and skill bodies | `cli/cmd/paths.go` (`resolveKimiHome`) |
...
**Problem:** Kimi shows no MCP servers at all after an install · **Cause:** one entry in `mcp.json` is invalid, which makes Kimi reject the whole file — usually an entry devexp did not write, and the install warns about it · **Fix:** read `$KIMI_CODE_HOME/logs/kimi-code.log` for the entry it names, fix or remove it, and re-run. `kimi doctor` will not find it: it checks `config.toml` and `tui.toml` only.
