...
| `KIMI_CODE_HOME` | No | `~/.kimi-code` | Root of the Kimi Code install destinations. Must be absolute, and may be neither `/` nor `$HOME`; anything else is refused. `mcp.json` and the manifest are written there; agents, skills and hooks are not, yet (#113, #114) | `cli/cmd/paths.go` (`resolveKimiHome`) |
...
**Problem:** `nothing was installed: Kimi Code CLI installs MCP servers only so far` · **Cause:** Kimi Code was the only selected target and the run asked for `--agents-only` or `--skills-only`, which Kimi does not install until #113 · **Fix:** drop the flag, or select another target too.
