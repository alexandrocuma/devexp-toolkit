...
./install.sh --dry-run                # preview; installs nothing   # source: install.sh:20, cli/cmd/install.go:117
```
...
| Install — non-interactive | `./install.sh` with any of `--dry-run`, `--reinstall-mcps`, `--mcps-only`, `--agents-only`, `--skills-only` (`--model` alone does **not** skip the wizard) | `cli/cmd/install.go:117-121` |
...
Configuration file: `devexp.config.json` at the repo root (model default, disabled agents/skills/hooks, extra MCPs). A missing file is a warning, not an error (`cli/cmd/install.go:97-100`). Schema and team usage: [`../guides/team-distribution.md`](../guides/team-distribution.md).
...
**Problem:** the installer opens an interactive wizard (or fails without a TTY) · **Cause:** none of `--dry-run`, `--reinstall-mcps`, `--mcps-only`, `--agents-only`, `--skills-only` was passed — `--model` alone doesn't count (`cli/cmd/install.go:117-121`) · **Fix:** pass one of those flags for the non-interactive path.
...
**Problem:** a downloaded release binary uses your clone's files, or a test binary picks up the wrong assets · **Cause:** asset lookup order is `$DEVEXP_DIR` → the directory above the executable → walk up from cwd for a dir containing `agents/`, `skills/` and `mcps/`; only if none match does it extract embedded assets to `<user cache dir>/devexp/assets` (`cli/internal/repo/repo.go:45-63,93`) · **Fix:** set `DEVEXP_DIR` explicitly, or run from outside the clone. A standalone run prints `Running standalone — using assets bundled in this binary.` (`cli/cmd/install.go:93`).
