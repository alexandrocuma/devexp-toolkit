...
./install.sh --dry-run                # preview; installs nothing   # source: install.sh:20, cli/cmd/install.go:108
```
...
| Preview an install | `./install.sh --dry-run` (or `-n`) | `cli/cmd/install.go:31` |
| Install — interactive wizard | `./install.sh` (no flags; needs a TTY) | `cli/cmd/install.go:108-144` |
| Install — non-interactive | `./install.sh` with any of `--dry-run`, `--reinstall-mcps`, `--mcps-only`, `--agents-only`, `--skills-only` (`--model` alone does **not** skip the wizard) | `cli/cmd/install.go:108-112` |
...
| `HOME` | Yes | from shell | Root of every install destination (`~/.claude/…`, `~/.config/opencode/…`) | `cli/cmd/install_claude.go:20`, `cli/cmd/install_opencode.go:18`, `cli/cmd/paths.go` |
...
Configuration file: `devexp.config.json` at the repo root (model default, disabled agents/skills/hooks, extra MCPs). A missing file is a warning, not an error (`cli/cmd/install.go:88-91`). Schema and team usage: [`../guides/team-distribution.md`](../guides/team-distribution.md).

## Troubleshooting

**Problem:** `go build`/`go test`/`go vet` fails with `internal/assets/assets.go:16:12: pattern all:agents: no matching files found` · **Cause:** the `//go:embed` dirs under `cli/internal/assets/` are gitignored staging output (`.gitignore:6-11`), absent in a fresh clone or worktree · **Fix:** `./scripts/stage-assets.sh`, then retry.
...
**Problem:** the installer opens an interactive wizard (or fails without a TTY) · **Cause:** none of `--dry-run`, `--reinstall-mcps`, `--mcps-only`, `--agents-only`, `--skills-only` was passed — `--model` alone doesn't count (`cli/cmd/install.go:108-112`) · **Fix:** pass one of those flags for the non-interactive path.
...
**Problem:** a downloaded release binary uses your clone's files, or a test binary picks up the wrong assets · **Cause:** asset lookup order is `$DEVEXP_DIR` → the directory above the executable → walk up from cwd for a dir containing `agents/`, `skills/` and `mcps/`; only if none match does it extract embedded assets to `<user cache dir>/devexp/assets` (`cli/internal/repo/repo.go:45-63,93`) · **Fix:** set `DEVEXP_DIR` explicitly, or run from outside the clone. A standalone run prints `Running standalone — using assets bundled in this binary.` (`cli/cmd/install.go:84`).
