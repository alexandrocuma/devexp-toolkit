...
- **Test seams are passed-in functions, not interfaces.** The repo has no interfaces and no DI container. For example, `removeStale(dir string, old, installed []string, shape staleShape, removeFn func(path string) error, dryRun bool) (kept []string)` in `cli/cmd/backup.go` receives `os.Remove` or `os.RemoveAll`, and `var userCacheDir = os.UserCacheDir` in `cli/internal/repo/repo.go` is a package variable that tests swap.
