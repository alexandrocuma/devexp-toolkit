...
- **Loading state never blocks an install.** A missing manifest counts as empty. An unreadable or malformed one also counts as empty, with a warning, and no agent or skill is treated as stale on that run (opencode plugin files are also recognised on disk) (`manifest.Load` in `cli/internal/manifest/manifest.go`, `loadOldManifest` in `cli/cmd/backup.go`). Malformed `settings.json` or opencode `config.json` is read with `json.Unmarshal(...) //nolint:errcheck` and treated as empty (`cli/internal/hooks/installer.go`, `cli/internal/mcp/opencode.go`). A missing `devexp.config.json` warns and uses the defaults (`cli/cmd/install.go`). Backup failures are ignored (`cli/cmd/backup.go`). The reason is recorded in the doc comments:
  ...
  **The limit on that policy.** Degrading is allowed; degrading *silently* is not. A discarded error is acceptable only where a comment states why **and** the caller still reports the degraded state to the user. The failure this bounds is not dropping the error — it is dropping it invisibly, so that an unreadable registry is indistinguishable from a build that ships no MCPs, and the symptom points nowhere near the cause.

  Mechanically, `errcheck` and `nolintlint` enforce the first half in `.golangci.yml`: an unchecked error fails, and a bare `//nolint:errcheck` fails too, so every suppression has to name what the user still sees. The second half — whether the degraded state is actually *reported* — cannot be checked by a linter and is a review question in `/deliver`'s Phase 5: **does any failure here collapse into a value that looks like success?** That question is deliberately language-neutral, because the linter only covers Go and only covers discarded errors, while the same defect appears as a nil default standing in for an error, a function that exits 0 without doing its work, and a guard that returns allow without proving it ran.
