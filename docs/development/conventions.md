...
| Hook JS exports | lowerCamel form of the hook name, as `export async function <name>(_ctx)` | `dangerousCmdGuard` in `hooks/opencode/dangerous-cmd-guard.js`; `lintOnSave` in `hooks/opencode/lint-on-save.js` |
...
- **Test seams are passed-in functions, not interfaces.** The repo has no interfaces and no DI container. For example, `removeStale(dir string, stale []string, removeFn func(path string) error, dryRun bool)` in `cli/cmd/backup.go` receives `os.Remove` or `os.RemoveAll`, and `var userCacheDir = os.UserCacheDir` in `cli/internal/repo/repo.go` is a package variable that tests swap.
...
- `[INCONSISTENT — hooks with mirrored sh+js unit tests (secret-guard, dangerous-cmd-guard) vs hooks covered only by the shell fail-closed test (secret-in-write-guard, large-file-guard, lint-on-save, format-on-save, test-on-save) or by nothing (the three graphify-* hooks)]`. When you change a hook's decision logic, add mirrored tests; see [testing](testing.md).
