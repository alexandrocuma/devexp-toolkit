...
Two things are enforced, by the `lint` job in `.github/workflows/ci.yml`. `gofmt` and revive's `exported` rule cover the Go CLI (`.golangci.yml`, run by `scripts/golangci-lint.sh`), deliberately narrow — the default linter set is off and widening is a decision, not a default. The comment standard is checked by `scripts/check-comment-refs.sh`, which is language-agnostic because the rule is: a Go linter could only ever hold it in `cli/` and would quietly abandon it in the shell and JS hooks. `//nolint:errcheck` marks errors that are ignored on purpose (`cli/cmd/backup.go`, `cli/internal/hooks/installer.go`, `cli/internal/mcp/opencode.go`).
