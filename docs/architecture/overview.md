...
| cobra, viper, promptui | Commands; reading `devexp.config.json`; the interactive wizard (needs a TTY — `chooseInstallMode` skips it without one) | `cli/cmd/root.go`, `cli/internal/config/config.go`, `cli/internal/ui/prompts.go` |
