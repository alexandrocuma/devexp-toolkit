...
- **The cut's generic fallback is deliberately plain:** with no Cut definition, `/release` creates a published release with the tag verified (`--verify-tag`) and the changelog section passed as a notes **file**, never an inline `--notes` string. It never adds `--draft` on its own — a repo with no publisher step would leave the draft unpublished forever
