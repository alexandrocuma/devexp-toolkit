...
**Don't commit `graphify-out/`.** The self-gating cuts both ways: `graphify-read-guard`'s only precondition is `existsSync('graphify-out/graph.json')` — `if (!isSource || inGraphify || !graphExists) return;` — and it never checks that the `graphify` CLI is installed. Because the hook is `opencode.enabled: true`, committing a graph would arm the gate for every contributor who clones the repo and works in opencode: their first source-file read blocks until they run 5 `graphify query` calls, and a contributor without graphify installed has no way to clear it. Upstream graphify recommends committing `graphify-out/` so a team shares one map; that advice is sound for a repo that doesn't also ship this hook. This one does, so `graphify-out/` is gitignored and each person builds their own with `/graphify`.
