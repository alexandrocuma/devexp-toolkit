...
**Kimi Code CLI** runs the *same* Claude Code scripts, through `hooks/kimi/adapter.sh`, registered as a marked `[[hooks]]` block in `$KIMI_CODE_HOME/config.toml`. The scripts are **copied** into `$KIMI_CODE_HOME/hooks/` rather than run from the checkout, because Kimi reads a command it cannot run as an *allow* (`cli/internal/hooks/kimi_install.go`). The adapter translates Kimi's envelope into Claude Code's and is fail-closed: anything it cannot carry out faithfully ends at `exit 2`. **Kimi's envelope is snake_case at the top level and nowhere else** — `runMatchedHooks` runs `toHookInputData` (a one-level `camelToSnake`) over the whole payload before `runHook` spawns the command, so `tool_name` and `tool_input` already read the way the guards want, while `tool_input` keeps the tool's own argument names. What is left for the adapter is the nested rename (a tool's `path` → `file_path`) and `ReadMediaFile` → `Read`; it applies `camelToSnake` at the top level too, which is a no-op on what Kimi sends and covers an envelope that ever arrives unconverted. Three of Kimi's traits shape what may be installed at all:
...
**`graphify-out/` is committed here, and `graphify-read-guard` is off because of it.** `graph.json` and `manifest.json` are tracked so a clone — or a new ticket [worktree](../guides/worktree-per-ticket.md), which starts empty of ignored files — has the map without paying a full extraction to rebuild it; `manifest.json` keys are relative, so the cache still matches after a move, and a code-only `--update` re-extracts without any LLM call. The catch is that `graphify-read-guard`'s only precondition is `existsSync('graphify-out/graph.json')` — `if (!isSource || inGraphify || !graphExists) return;` — and it never checks that the `graphify` CLI is on PATH. `./install.sh` ships the graphify *skill*, not the CLI, so with a graph committed and the guard enabled a contributor without `graphify` installed is blocked on their first source-file read with no way to clear it. Its `opencode.enabled` is therefore `false`, which is also what its own registry description always claimed ("Ships disabled"). `graphify-grep-nudge` and `graphify-session-sentinel` stay on: neither can block — one only adds `additionalContext`, the other only increments counters.
