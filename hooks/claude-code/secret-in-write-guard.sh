...
if ! . "$devexp_dir/scan-budget.sh"; then
    echo "[devexp secret-in-write-guard] internal error -- the scan budget could not be loaded, so the guard did not run. Blocking to be safe." >&2
    exit 2
fi
devexp_scan_budget secret-in-write-guard "$@"
